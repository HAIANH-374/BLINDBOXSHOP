...
class _ProductsPageState extends ConsumerState<ProductsPage>
  ...
  String? _mapCategoryName(String? displayName) {
    ...
    final categoriesAsync = ref.read(activeCategoriesProvider(null));

    return categoriesAsync.when(
      data: (categories) {
        final category = categories.firstWhere(
          (c) => c.name == displayName,
          orElse: () => categories.first, // dự phòng
        );
        return category.id;
      },
      loading: () {
  ...
  List<ProductEntity> _applyFiltersAndSort(List<ProductEntity> products) {
    Iterable<ProductEntity> result = products;
  ...
  Widget _buildLoadingSkeleton() {
    ...
    final aspectRatio = columns == 2 ? 0.50 : (columns == 3 ? 0.85 : 1.0);

    // Lưới skeleton khi đang tải
  ...
  Widget buildGrid(List<ProductEntity> products) {
    if (products.isEmpty) {
  ...
  Widget _buildFiltersBar() {
    final categoriesAsync = ref.watch(activeCategoriesProvider(null));
    final brandsAsync = ref.watch(brandsProvider);

    // Thanh bộ lọc ngang (danh mục, thương hiệu, reset, sắp xếp)

...
class _AdminOrderManagementPageState
  ...
  // Thêm key để force rebuild khi filter thay đổi
  int _filterKey = 0;
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                _buildDetailRow(
                  'Trạng thái',
                  _getStatusDisplayName(order.status.name),
                ),
                _buildDetailRow('Tổng tiền', order.formattedTotalAmount),
                _buildDetailRow('Ngày tạo', _formatDateTime(order.createdAt)),
                _buildDetailRow(
                  'Phương thức thanh toán',
                  order.paymentMethodName ?? '—',
                ),
  ...
  Future<void> _updateOrderStatus(
    ...
    try {
      ...
      if (mounted) {
        Navigator.of(context).pop();
        NotificationUtils.showError('Lỗi cập nhật trạng thái: ${e.toString()}');
      }
    }
  }
